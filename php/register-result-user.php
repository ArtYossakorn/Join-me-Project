...
$ref_table_register = "register";  // ตารางที่เก็บการลงทะเบียน

// ดึงข้อมูลการลงทะเบียนของผู้ใช้
$registrations = $database->getReference($ref_table_register)
    ->orderByChild("userID")
    ->equalTo($uid)
    ->getValue();

$registeredPostIDs = [];  // เก็บรายการ postID ที่ user ลงทะเบียน

// ตรวจสอบว่าผู้ใช้มีการลงทะเบียนหรือไม่
if (!empty($registrations)) {
    foreach ($registrations as $regID => $registration) {
        if (isset($registration['postID'])) {
            $registeredPostIDs[] = $registration['postID'];  // เก็บ postID ที่ user ลงทะเบียน
        }
    }
}
    ...
    <section class="dashboard">
        ...
        <div class="dash-content">
            <div class="title">
                ...
                <span class="text">กิจกรรมที่ลงทะเบียน</span>
            </div>
        </div>
        ...
        <div class="register-row">
            ...
            if (!empty($posts)) {
                $posts = array_reverse($posts); // ให้โพสต์ล่าสุดอยู่บนสุด

                foreach ($posts as $postID => $post) {
                    // แสดงเฉพาะโพสต์ที่ผู้ใช้ลงทะเบียน
                    if (in_array($postID, $registeredPostIDs)) {
                        if (isset($post['postName'], $post['description'], $post['location'], $post['date'], $post['startTime'], $post['endTime'])) {
                            ...
                            // อัปเดตสถานะกิจกรรมในฐานข้อมูล Firebase
                            if ($status == "เสร็จสิ้น") {
                                // อัปเดตสถานะในตาราง post
                                $database->getReference($ref_table_post . '/' . $postID)
            ...
            <?php
                ...
                echo "<p>ยังไม่มีกิจกรรมที่คุณลงทะเบียน.</p>";
            }
            ?>
        </div>
